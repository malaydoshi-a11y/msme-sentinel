...
to route around. In IDBI's own AMA session for this track, a participant separately
pointed out that *raw* accuracy is a misleading metric on an imbalanced default dataset —
defaults are a small minority class, so a model that simply predicts "no default" for
every account can score above 90% raw accuracy while providing zero business value.
IDBI's response acknowledged this and effectively invited teams to measure "accuracy"
correctly rather than being bound to the naive version of it.

So we do both: we answer the literal ask, and we answer it correctly.

## Primary metric: Balanced Accuracy — 81.4%
...
ignoring the minority class. We report **81.4%**, at the threshold (0.41) that maximizes
this metric on held-out data. For contrast: naive accuracy at that same threshold is
77.95% — lower here specifically because the model isn't just defaulting to the majority
class, which is the honest, non-inflated number for a properly imbalance-aware model.

## Supporting rigor metrics — why 81.4% can be trusted

- **AUC-ROC (0.885)** — the standard measure of a model's ability to rank a defaulting
  account higher-risk than a non-defaulting one, robust to class imbalance.
- **KS-Statistic (0.629)** — the classic credit-scoring separation metric (maximum distance
...
- **Gini Coefficient (0.770)** — `2 × AUC − 1`, the standard scorecard-quality benchmark used
  across Indian and global credit bureaus.
- **Recall @ 0.30 threshold (0.895)** — a deliberately *different*, more sensitive operating
